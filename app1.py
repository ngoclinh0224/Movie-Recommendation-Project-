...
if st.button("Recommend"):
    ...
    if movie_name.lower() not in title_to_id:
        ...
        if model_choice == "BERT":
            final_scores = bert_scores_scaled

        elif model_choice == "ALS":
            final_scores = als_scores_scaled

        else:
             final_scores = hybrid_scores
        
        # loại seed movie khỏi kết quả
        ...
        recommendations = (df_final.iloc[top_indices][["title"]].copy())
        ...
        st.markdown(
            f"""
            <h3>
            🎬 Top {TOP_K} Recommendations ({model_choice})
            </h3>
            """,
            unsafe_allow_html=True
        )
